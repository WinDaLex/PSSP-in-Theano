...
train = snd_layer_classifier.train(X_train, Y_train)
valid = snd_layer_classifier.predict(X_valid)
...
for i in range(num_epochs):
    ...
    print 'epoch %3d\%d. train cost: %f, Q3_valid: %.3f%%.' % \
        (i+1, num_epochs, np.mean(costs), A_valid.Q3)
